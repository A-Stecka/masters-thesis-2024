...
#include "method/operators/mutation/mutations/CDifferentialEvolution.h"
...
AMutation *CMutationFactory::Create(SConfigMap *configMap, const std::string& configKey, AProblem &problem)
{
    ...
    if (strcmp(opName, "RandomBit") == 0 && (encodingTypes.find(EEncodingType::ASSOCIATION) != encodingTypes.end() || encodingTypes.find(EEncodingType::PERMUTATION) != encodingTypes.end()))
    {
        ...
        return new CRandomBit("RandomBit", mutProb);
    }
    else if (strcmp(opName, "TTP_Reverse_Flip") == 0 && encodingTypes.find(EEncodingType::PERMUTATION) != encodingTypes.end())
    {
        ...
        return new CTTPReverseFlip("TTP_Reverse_Flip", mutProb, etaCoef);
    }
    else if (strcmp(opName, "CVRP_Reverse_Flip") == 0 && encodingTypes.find(EEncodingType::PERMUTATION) != encodingTypes.end())
    {
        float mutProb = std::stof(vec[1]);
        return new CCVRPReverseFlip("CVRP_Reverse_Flip", mutProb);
    }
    // Mutation operator added for CCoDE
    else if (strcmp(opName, "DifferentialEvolution") == 0 && encodingTypes.find(EEncodingType::ASSOCIATION) != encodingTypes.end())
    {
        float mutFac = std::stof(vec[1]);
        return new CDifferentialEvolution("DifferentialEvolution", mutFac);
    }
    else if (strcmp(opName, "CheapestResourceMutation") == 0)
    {
        float mutProb = std::stof(vec[1]);
        return new CCheapestResourceMutation("CheapestResourceMutation", mutProb, dynamic_cast<CMSRCPSP_TA&>(problem));
    }
    else if (strcmp(opName, "LeastAssignedResourceMutation") == 0)
    {
        float mutProb = std::stof(vec[1]);
        return new CLeastAssignedResourceMutation("LeastAssignedResourceMutation", mutProb, dynamic_cast<CMSRCPSP_TA&>(problem));
    }
    return nullptr;
}
