...
#include "methods/MO/CCoDE/CCCoDEFactory.h"
#include "methods/MO/CCoDE/CRandomMethodFactory.h"
...
)
{
    ...
    // Check if Cooperative Co-evolution method is being run with the proper problem encoding
    if (strcmp(methodName.c_str(), "CCoDE") == 0 &&
        !std::set<std::string>{"MSRCPSP_CC_R", "MSRCPSP_CC_S", "MSRCPSP_CC_R_NA", "MSRCPSP_CC_S_NA", "MSRCPSP_CC_SR", "MSRCPSP_NO_COEV", "MSRCPSP_SS"}
        .count(problemName))
    {
        throw std::runtime_error("Method CCoDE can only be run for problems: MSRCPSP_CC_R, MSRCPSP_CC_S, MSRCPSP_CC_R_NA, "
                                 "MSRCPSP_CC_S_NA, MSRCPSP_CC_SR, MSRCPSP_NO_COEV, MSRCPSP_SS");
    }
    // Check if double population encoding is being solved by the proper method
    if (std::set<std::string>{"MSRCPSP_CC_R", "MSRCPSP_CC_S", "MSRCPSP_CC_R_NA", "MSRCPSP_CC_S_NA", "MSRCPSP_CC_SR", "MSRCPSP_NO_COEV", "MSRCPSP_SS"}
        .count(problemName) &&
        !std::set<std::string>{"CCoDE", "Random"}.count(methodName))
    {
        throw std::runtime_error(std::string(problemName) + " problem can only be solved by methods: CCoDE, Random");
    }
    ...
    if (strcmp(methodName.c_str(), "Random") == 0)
        return CRandomMethodFactory::CreateRandomMethod(configMap, problem, initialization);
    ...
    if (strcmp(methodName.c_str(), "CCoDE") == 0)
        return CCCoDEFactory::CreateCCoDE(configMap, problem, initialization, crossover, mutation);
